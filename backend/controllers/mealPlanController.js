    ...
    try {
        ...
        if (!startDate) {
            return res.status(400).json({
                success: false,
                message: 'Please provide start_date query parameter'
            });
        }

        const mealPlans = await MealPlan.getWeaklyPlan(req.user.id, startDate);
...
// FIX: was deleteMealPantry (wrong name — should be deleteMealPlan)
