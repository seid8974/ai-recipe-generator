...
import { ChefHat, UtensilsCrossed, Calendar, Clock, Flame } from 'lucide-react';
...
const Dashboard = () => {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            const [recipesRes, pantryRes, mealPlanRes, recentRes, upcomingRes, todayMealsRes] = await Promise.all([
                ...
                api.get(`/meal-plan/weekly?start_date=${today}&end_date=${today}`),
            ]);
            ...
            // Sum calories from today's meal plan
            const todayMeals = todayMealsRes.data.data.mealPlans || [];
            const cal = todayMeals.reduce((sum, m) => sum + (parseInt(m.calories) || 0), 0);
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="mb-8">
                    ...
                    <p className="text-gray-600 mt-1">Here's your cooking overview for today</p>
                </div>

                {/* Stats Cards */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <StatCard icon={<ChefHat className="w-6 h-6" />} label="Total Recipes" value={stats.totalRecipes} color="emerald" />
                    <StatCard icon={<UtensilsCrossed className="w-6 h-6" />} label="Pantry Items" value={stats.pantryItems} color="blue" />
                    <StatCard icon={<Calendar className="w-6 h-6" />} label="Meals This Week" value={stats.mealsThisWeek} color="purple" />
                </div>
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-6 mb-8">
                    <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-lg font-semibold text-gray-900">Today's Calorie Goal</h2>
                        </div>
                        <div className="text-right">
                            <span className="text-2xl font-bold text-gray-900">{todayCalories}</span>
                            <span className="text-gray-500 text-sm"> / {calorieGoal} kcal</span>
                        </div>
                    </div>
                    <div className="w-full bg-gray-100 rounded-full h-3">
                        <div
                            className={`h-3 rounded-full transition-all duration-500 ${
                                ...
                                caloriePercent >= 75 ? 'bg-orange-500' :
                                'bg-emerald-500'
                            }`}
                    ...
                    {todayCalories === 0 && (
                        <p className="text-xs text-gray-400 mt-2">
                            ...
                            <Link to="/meal-plan" className="text-emerald-600 hover:underline">Plan your meals →</Link>
                        </p>
                    )}
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 p-6">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <Link to="/recipes" className="text-sm text-emerald-600 hover:text-emerald-700 font-medium">View all</Link>
                        </div>
                            ...
                            <div className="space-y-3">
                                {recentRecipes.map((recipe) => (
                                    <Link key={recipe.id} to={`/recipes/${recipe.id}`}
                                        ...
                                        <div className="w-12 h-12 bg-emerald-100 rounded-lg flex items-center justify-center overflow-hidden shrink-0">
                                            {recipe.image_url
                                                ? <img src={recipe.image_url} alt={recipe.name} className="w-full h-full object-cover" onError={(e) => { e.target.style.display='none'; }} />
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 p-6">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <Link to="/meal-plan" className="text-sm text-emerald-600 hover:text-emerald-700 font-medium">View calendar</Link>
                        </div>
                        {upcomingMeals.length > 0 ? (
                            <div className="space-y-3">
                                {upcomingMeals.map((meal) => (
                                    <div key={meal.id} className="flex items-center gap-3 p-3 rounded-lg border border-gray-100">
